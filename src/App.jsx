...
export default function Portfolio() {
  ...
  const navItem = (id, label) => (
    <a
      href={`#${id}`}
      className={`hover:text-cyan-400 transition ${active === id ? "text-cyan-400 font-bold" : ""}`}
    >
      {label}
    </a>
  );
  ...
  return (
    <div className={dark ? "bg-black text-white min-h-screen scroll-smooth" : "bg-white text-black min-h-screen scroll-smooth"}>
      ...
      {/* HERO */}
      <section id="home" className="h-screen flex flex-col justify-center items-center text-center px-6 relative overflow-hidden">
        ...
        <div className="mt-8 flex gap-4 z-10">
          <a href="#contact" className="px-6 py-2 bg-cyan-500 text-black font-semibold rounded-xl hover:scale-105 transition">
            Connect
          </a>
        </div>
      </section>
      ...
      <section id="about" className="p-16 text-center">
        ...
        <p className="max-w-3xl mx-auto opacity-80">
          Results-driven B.Tech Data Science graduate with hands-on experience in ML, AI, and cloud systems.
          Built real-world pipelines processing 10,000+ records and achieved 95%+ model accuracy.
        </p>
      </section>
      ...
      <section id="skills" className="p-16 text-center">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {["Python | SQL | Pandas", "Power BI | Excel | Visualization", "AWS | Docker | Git"].map((s, i) => (
            <motion.div key={i} whileHover={{ scale: 1.05 }} className={cardStyle}>
              {s}
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {projects.map((p, i) => (
            <motion.div key={i} whileHover={{ scale: 1.05 }} className={cardStyle}>
              ...
              <a href={p.github} target="_blank" className="inline-block mt-4 px-3 py-1 border rounded hover:bg-cyan-500">
                GitHub
              </a>
            </motion.div>
          ))}
